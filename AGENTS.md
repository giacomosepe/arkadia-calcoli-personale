...
This project was designed and evolved across these Codex.ai conversations:
- Original build: https://Codex.ai/chat/ff965a17-c8cd-4140-a64d-baab61bc1f38
- Prompt engineering + architecture refinement: https://Codex.ai/share/60303975-a198-4802-bc57-3d32339d433f
...
1. Extract daily worked hours from Italian payroll PDFs (LUL — Libro Unico del Lavoro) using the Codex API
...
- **Anthropic SDK** — `Codex-sonnet-4-5-20250929` (always pin the full model string)
- **pdf-lib** — splits multi-page PDFs into single pages before sending to Codex
...
server/
  api/
    extract.post.ts            ← splits PDFs, calls Codex once per page, verifies totals
  ...
  utils/
    Codex.ts                  ← SYSTEM_PROMPT + buildUserPrompt() + Anthropic API call
...
Calls `Codex.ts` once per page → one JSON record per page.
...
5. Each page sent to Codex separately (1 page = 1 employee-month)
6. Codex returns structured JSON (see prompt section below)
...
## The Codex prompt (`server/utils/Codex.ts`)

> ⚠️ **DO NOT EDIT THE PROMPTS IN `Codex.ts` WITHOUT EXPLICIT OWNER APPROVAL**
> This applies to both Codex.ai and Codex.
...
**Only return days with hours > 0** — Codex omits zero-hour days (weekends, holidays).
...
**Hours as decimal in JSON** — Codex returns 8.75, not "8:45".
...
**Model string is pinned** — always `Codex-sonnet-4-5-20250929`, never the
alias `Codex-sonnet-4-5`. Aliases can silently change.

**max_tokens: 2048** — Codex returns only worked days (typically 20–22 per month).
Response is under 400 tokens in practice. 2048 is safe headroom without waste.

### Current prompts (source of truth is Codex.ts — these are for reference)

```
SYSTEM_PROMPT extract from file /utils/Codex.ts : 
...
- Compare to `declared_total` from Codex
